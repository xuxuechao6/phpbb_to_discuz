...
elseif ($step == '4')
{
    ...
    $query = $SDB->query("SELECT m.uid,m.username,m.email,m.password,m.adminid,m.groupid,m.groupexpiry,m.extgroupids,m.regdate,m.credits," .
			...
			"ms.regip,ms.lastip,ms.lastvisit,ms.lastactivity,ms.lastpost,ms.lastsendmail,ms.notifications,ms.groupinvitations,ms.activityinvitations," .
	...
	while ($m = $SDB->fetch_array($query))
	{
		...
		*/
        ...
        if(!$speed)//一条一条插
        {
            ...
            $DDB->update("REPLACE INTO {$pw_prefix}memberdata (uid,postnum,digests,rvrc,money,credit,currency,lastvisit,thisvisit,lastpost) VALUES (".$m['uid'].",".$m['posts'].",".$m['digestposts'].",".$rvrc.",".$money.",".$credit.",".$currency.",'".$m['lastvisit']."','".$m['lastactivity']."','".$m['lastpost']."') ");
        }
        ...
        if($speed){//批量插
            ...
            $memdatasql[] = "(".$m['uid'].",".$m['posts'].",".$m['digestposts'].",".$rvrc.",".$money.",".$credit.",".$currency.",'".$m['lastvisit']."','".$m['lastactivity']."','".$m['lastpost']."')";
        }
		$s_c++;
	}
    ...
    if($speed){//批量插
        ...
        if($memdatasql)
        {
            ...
            $DDB->update("REPLACE INTO {$pw_prefix}memberdata (uid,postnum,digests,rvrc,money,credit,currency,lastvisit,thisvisit,lastpost) VALUES $memdatastr ");
        }
    }
...
elseif ($step == '6')
{
	...
	$query = $SDB->query("SELECT t.tid,t.typeid,t.subject,t.readperm,t.price,t.lastpost,t.lastposter,t.views,t.replies,t.displayorder,t.highlight," .
			"t.digest,t.special,t.attachment,t.moderated,t.closed," .
			...
			FROM {$source_prefix}forum_thread t force index(PRIMARY)
...
elseif ($step == '911')
{
	...
	$query = $SDB->query("SELECT p.*, t.dateline FROM {$source_prefix}forum_poll p " .
			"LEFT JOIN {$source_prefix}forum_thread t USING (tid) WHERE p.tid >= $start ORDER BY p.tid LIMIT $percount");
	while($v = $SDB->fetch_array($query))
	{
        $lastid = $v['tid'];
		$votearray = array();
...
elseif ($step == '12')
{
	...
	$query = $SDB->query("SELECT * FROM {$source_prefix}forum_post where pid > $start order by pid LIMIT $percount");
    ...
    $maxid = $SDB->get_value("SELECT max(pid) FROM {$source_prefix}forum_post");
    echo '最大id',$maxid,'<br>','最后id',$lastid;
    if($maxid > $lastid)
    {
		refreshto($cpage.'&step='.$step.'&start='.$end.'&f_c='.$f_c.'&s_c='.$s_c);
...
*/
	while ($rt = $SDB->fetch_array($query))
	{
        ...
        $DDB->update("UPDATE {$pw_prefix}threads SET fid = ".$newfid.",tpcstatus=1 WHERE tid = ".$rt['tid']);
        $DDB->update("UPDATE {$pw_prefix}posts SET fid = ".$newfid." WHERE tid = ".$rt['tid']);
