...
$query_topic = $DDB->query("SHOW TABLE STATUS LIKE '{$pw_prefix}topicvalue%'");
while ($topic = $DDB->fetch_array($query_topic))
{
    $DDB->update("drop table ".$topic['Name']);
}
$DDB->update("truncate table {$pw_prefix}topiccate");
$DDB->update("truncate table {$pw_prefix}topicfield");
$DDB->update("truncate table {$pw_prefix}topicmodel");
...
foreach ($topicdata as $cate) {
	...
	$DDB->update("REPLACE INTO {$pw_prefix}topiccate SET " . pwSqlSingle($cate));
	...
	foreach ($models as $model) {
		...
		$DDB->update("REPLACE INTO {$pw_prefix}topicmodel SET " . pwSqlSingle($model));
		$tablename = "{$pw_prefix}topicvalue".$modelid;
		...
		foreach ($fields as $field) {
			...
			$DDB->update("REPLACE INTO {$pw_prefix}topicfield SET ".pwSqlSingle($field,false));
		...
		$DDB->query("DROP TABLE IF EXISTS $tablename");
