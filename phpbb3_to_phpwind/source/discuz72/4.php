...
//会员
$banusersql = $banuids = array();//禁言会员处理数据初始化
...
$query = $SDB->query("SELECT m.uid,m.username,m.password,m.secques,m.gender,m.adminid,m.groupid,m.groupexpiry,
					  m.extgroupids,m.regip,m.regdate,m.lastip,m.lastvisit,m.lastactivity,m.lastpost,m.posts,
					  m.digestposts,m.oltime,m.pageviews,m.credits,m.extcredits1,m.extcredits2,m.extcredits3,
					  m.extcredits4,m.extcredits5,m.extcredits6,m.extcredits7,m.extcredits8,m.email,m.bday,
					  m.sigstatus,m.tpp,m.ppp,m.styleid,m.dateformat,m.timeformat,m.pmsound,m.showemail,
					  m.newsletter,m.invisible,m.timeoffset,m.accessmasks,m.editormode,
        			  mf.nickname,mf.site,mf.alipay,mf.icq,mf.qq,mf.yahoo,mf.msn,mf.taobao,mf.location,
        			  mf.customstatus,mf.medals,mf.avatar,mf.avatarwidth,mf.avatarheight,mf.bio,mf.sightml,
        			  mf.ignorepm,mf.groupterms,mf.authstr,mf.spacename,mf.buyercredit,mf.sellercredit".$querysql.",
...
while ($m = $SDB->fetch_array($query)){
    Add_S($m);
    $lastid = $m['uid'];
    if (empty($m['uid']) || empty($m['username'])){
        $f_c++;
        errors_log($m['uid']."\t".$m['username']);
        continue;
    }
    //判断用户是否有新短信
    $newpm = $UCDB->get_value("SELECT COUNT(pmid) FROM {$uc_db_prefix}pms 
    						   WHERE msgtoid=".$m['uid']." AND new='1'");
    $newpm = ($newpm > 1) ? '1' : '0';
    ...
    if($groupid == '6'){
        ...
        if ($m['groupexpiry']){	//用户组有效期
            ...
            $banuids[] = $m['uid'];
            if(!$speed){
                $DDB->update("REPLACE INTO {$pw_prefix}banuser (uid,fid,type,startdate,days,admin,reason) 
                			  VALUES (".$m['uid'].",0,2,$timestamp,0,'','')");
            }
        }
    }
    ...
    if($expandCredit){ //自定义积分
        ...
        !empty($expandCreditSQL) && $DDB->update("INSERT INTO {$pw_prefix}membercredit (uid, cid, value) 
        										  VALUES ".substr($expandCreditSQL, 0, -1));
    }
    ...
    if($expandMember){ //自定义积分
        ...
        !empty($expandMemberSQL1) && $DDB->update("INSERT INTO {$pw_prefix}memberinfo (uid".$expandMemberSQL1.") 
        										   VALUES (".$m['uid'].$expandMemberSQL2.")");
    }
    ...
    //密码
    $uc = $UCDB->get_one("SELECT m.password,m.salt,mf.blacklist 
    					  FROM {$uc_db_prefix}members m 
    ...
    if(empty($speed)){ //一条一条插
        $DDB->update("REPLACE INTO {$pw_prefix}members (uid,username,password,email,groupid,groups,icon,gender,regdate,signature,introduce,oicq,icq,msn,yahoo,site,location,honor,bday,timedf,t_num,p_num,newpm,banpm,medals,userstatus,salt) VALUES (".$m['uid'].",'".$m['username']."','".$uc['password']."','".$m['email']."',".$groupid.",'".addslashes($m['extgroupids'])."','".$userface."','".$m['gender']."',".$m['regdate'].",'".$m['sightml']."','".$introduce."','".$m['qq']."','".$m['icq']."','".$m['msn']."','".$m['yahoo']."','".$m['site']."','".$m['location']."','".$m['customstatus']."','".$m['bday']."','".$timedf."','".$m['tpp']."','".$m['ppp']."',".$newpm.",'$banpm','$medals','".$userstatus."','".$uc['salt']."')");
...
if(!empty($speed)){//批量插
    if($membersql){
        $DDB->update("REPLACE INTO {$pw_prefix}members (uid,username,password,email,groupid,groups,icon,gender,regdate,signature,introduce,oicq,icq,msn,yahoo,site,location,honor,bday,timedf,t_num,p_num,newpm,banpm,medals,userstatus,salt) VALUES ".implode(",",$membersql));
    }
    if($memdatasql){
        $DDB->update("REPLACE INTO {$pw_prefix}memberdata (uid,postnum,digests,rvrc,money,credit,currency,lastvisit,thisvisit,lastpost,onlinetime,monoltime) VALUES ".implode(",",$memdatasql));
    }
}
