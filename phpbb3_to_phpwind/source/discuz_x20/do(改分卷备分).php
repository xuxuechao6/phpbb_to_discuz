...
if ($step == 1)
{
	...
	$historyposts = $SDB->get_one("SELECT svalue FROM {$source_prefix}common_setting WHERE skey = 'historyposts'");
	$historyposts = $historyposts['value'] ? explode("\t", $historyposts['value']) : array();
	$onlinerecord = $SDB->get_one("SELECT svalue FROM {$source_prefix}common_setting WHERE skey = 'onlinerecord'");
...
elseif ($step == 3)
{
	if (!$start)
	{
        ...
        $query = $SDB->query("SELECT medalid,name,image FROM {$source_prefix}forum_medal");
	...
	{
        $lastid = $l['id'];
        $awardee = $SDB->get_value("SELECT username FROM {$source_prefix}common_member WHERE uid=".$l['uid']);
...
elseif ($step == '10')
{
	...
	if(!$start)
	{
        $DDB->update("CREATE TABLE IF NOT EXISTS `pw_pcvalue2` (
...
");
    ...
    if($tidarr){
        $DDB->update("UPDATE pw_threads SET special=22 WHERE tid in (".implode(',',$tidarr).")");
    }
...
*/
	while ($rt = $SDB->fetch_array($query))
	{
        ...
        $DDB->update("UPDATE pw_threads SET fid = ".$newfid.",tpcstatus=1 WHERE tid = ".$rt['tid']);
        $DDB->update("UPDATE pw_posts SET fid = ".$newfid." WHERE tid = ".$rt['tid']);
        $DDB->update("REPLACE INTO pw_argument (tid,cyid,postdate,lastpost,topped) VALUES (".$rt['tid'].",".$rt['fid'].",'".$rt['dateline']."','".$rt['lastpost']."','".$rt['displayorder']."')");
    }
