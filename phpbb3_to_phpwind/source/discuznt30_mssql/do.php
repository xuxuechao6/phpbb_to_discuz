...
*  Update BY rickyleo(riqi@aliyun-inc.com)
*
*/
!defined('R_P') && exit('Forbidden!');
$db_table = $step_data[$step];
//echo $SDB->get_value("SELECT @@version");exit;//查sqlserver版本
...
if ($step == '1'){
	...
	while($rs = $SDB->fetch_array($query)){
		...
		$DDB->update("INSERT INTO {$pw_prefix}smiles (id,path,type,name,vieworder) VALUES(".$rs['id'].",'".$url."',".pwEscape($rs['type']).",'".$rs['code']."',".pwEscape($rs['displayorder']).")");
	...
	$query = $SDB->query("SELECT us.uid,CONVERT(varchar(255), us.username) AS username, us.credits, 
						  us.extcredits1, us.extcredits2, us.extcredits3,us.extcredits4,us.extcredits5,
						  us.extcredits6,us.extcredits7,us.extcredits8, us.password, us.email, 
						  us.showemail, us.gender, us.groupid, CONVERT(char, us.joindate, 20) AS joindate,
						  us.bday, us.newsletter, us.posts, us.digestposts, us.oltime, us.lastip,
					      CONVERT(char, us.lastvisit, 20) AS lastvisit, CONVERT(char, us.lastactivity, 20)
					      AS lastactivity, CONVERT(char, us.lastpost, 20) AS lastpost, us.tpp, us.ppp, 
					      us.newpm, CONVERT(varchar(255), uf.avatar) AS avatar, uf.avatarwidth, 
					      uf.avatarheight, CONVERT(text, uf.signature) AS signature, 
					      CONVERT(text, uf.bio) AS bio, uf.qq, uf.yahoo, uf.msn, uf.icq, 
					      CONVERT(varchar(255), uf.website) AS website, uf.medals,
					      CONVERT(varchar(255), uf.location) AS location, 
					      CONVERT(varchar(255), uf.customstatus) AS customstatus
	...
	$query = $SDB->query("SELECT f.fid AS fid,f.layer AS layer,f.parentid AS parentid,
						  f.parentidlist AS parentidlist,f.subforumcount,convert(varchar(255),f.name) AS name,
						  f.colcount,f.displayorder,f.topics,f.posts,f.modnewposts,f.jammer,f.disablewatermark,
						  f.topics,f.posts,f.todayposts,f.status,fd.icon,fd.attachextensions,fd.viewperm,
						  fd.postperm,fd.replyperm,fd.getattachperm,fd.postattachperm,convert(text,fd.moderators) AS moderators,
						  convert(text,fd.description) AS description, convert(text,fd.topictypes) As topictypes, 
						  fd.postbytopictype, fd.topictypeprefix, fd.redirect,fd.applytopictype
...
elseif ($step == '9')
{
	...
	while($m = $SDB->fetch_array($query))
	{
		...
		if (($m['msgtoid'] != $m['msgfromid']) && !$m['folder'] && $m['msgtoid']!='-1')
		{
	        ...
	        $message_sql[] = "('".$m['pmid']."',".$m['msgfromid'].",".pwEscape($m['msgfrom']).",'".$m['subject']."','".$m['subject']."','".serialize(array('categoryid'=>1,'typeid'=>100))."',".$postdatetime.",".$postdatetime.",'".serialize(array($m['msgto']))."')";
...
elseif ($step == '11')
{
	...
	while($p = $SDB->fetch_array($query))
	{
		...
		if($p['layer']=='0')
		{
			$DDB->update("REPLACE INTO {$pw_prefix}tmsgs (tid,aid,userip,ifsign,ifconvert,content) VALUES (".$p['tid'].",'".$p['attachment']."','".$p['ip']."',".$p['usesig'].",".((convert($message) == $message)? 1 : 2).",'".addslashes($message)."')");
...
elseif ($step == '13')
{
	...
	$query = $SDB->query("SELECT * FROM {$source_prefix}albums
						  WHERE albumid >= $start AND albumid <= $end 
						  ORDER BY albumid");
	$goon = 0;
	while ($rt = $SDB->fetch_array($query)) {
		$goon++;
		...
		$aid		=	$rt['albumid'];
		$aname		=	$rt['title'];
		$aintro		=	$rt['description'];
		$atype		=	0;
		$private	=	!$rt['password'] ? 0 : 3;
		$ownerid	=	$rt['userid'];
		$owner		=	$rt['username'];
		$photonum	=	$rt['imgcount'];
		$lastphotoinfo	=	$SDB->get_one("SELECT TOP 1 photoid,filename,postdate FROM {$source_prefix}photos WHERE albumid ={$rt['albumid']} ORDER BY photoid DESC");
		$lastphoto	=	"photo/".$lastphotoinfo['filename'];
		$lasttime	=	maketime($lastphotoinfo['postdate']);
		$lastpid 	=	$lastphotoinfo['photoid'];
		$crtime	 	=   maketime($rt['createdatetime']);
	...
	$query = $SDB->query("SELECT * FROM {$source_prefix}photos 
						  WHERE photoid >= $start AND photoid <= $end
						  ORDER BY photoid ASC");
	unset($lastid);
	while ($rt = $SDB->fetch_array($query)) {
		$s_c ++;
		$lastid		= 	$pid = $rt['photoid'];
		$aid		=	$rt['albumid'];
		$pintro		=	$rt['description'];
		$path		=	"photo/".$rt['filename'];
		$uploader	=	$rt['username'];
		$uptime		=	maketime($rt['postdate']);
		$hits		=	$rt['views'];
		$ifthumb	=	0;
		$c_num		=	$rt['comments'];
	...
	$maxid = $SDB->get_value("SELECT MAX(photoid) FROM {$source_prefix}photos");
	...
	//照片评论
	$query = $SDB->query("SELECT * FROM {$source_prefix}photocomments 
						  WHERE commentid >= $start AND commentid <= $end
						  ORDER BY commentid ASC");
	unset($lastid);
	while ($rt = $SDB->fetch_array($query)) {
		Add_S($rt);
		$s_c ++;
		$lastid = $commontid = $rt['commentid'];
		$uid = $rt['userid'];
		...
		$typeid = $rt['photoid'];
		$upid = '0';
		$postdate = maketime($rt['postdatetime']);
		$ifwordsfb = '1';
		$cnphotocomment[] 	= 	array($uid, $username, $title, $type, $typeid, $upid, $postdate, $ifwordsfb);
	}
	$SDB->free_result($query);
	!empty($cnphotocomment) && $DDB->update("REPLACE INTO {$pw_prefix}comment (uid,username,title,type,typeid,upid,postdate,ifwordsfb) VALUES ".pwSqlMulti($cnphotocomment));

	$maxid = $SDB->get_value("SELECT MAX(commentid) FROM {$source_prefix}photocomments");
	empty($lastid) && $lastid = $end;
...
//这个时间很诡异，数据库里边查出来是 2011/6/8 12:22:35 结果就变成2011 六月 8 12:22了
function maketime($t){
	$t = trim($t);
	$t = str_replace(
